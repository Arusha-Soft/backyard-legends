...
namespace JuiceUp
{
    ...
    public class UiJuiceAnimator : MonoBehaviour
    {
        ...
        {
            [Tooltip("If enabled, this element will be ignored and not included in animations.")]
        ...
        public FeelingPreset preset = FeelingPreset.Dramatic;

        [Header("Runtime Completion")]
        ...
        [HideInInspector]
        [Tooltip("Allows Play In/Out buttons to animate while the Editor is not in Play Mode. Always enabled.")]
        public bool allowPreviewInEditMode = true;
        ...
        [Header("Sound")]
        [Tooltip("Coming Soon...")]
        public bool enabledSound = false;

        [Header("Events")]
        [Tooltip("Invoked when animation starts (Play In or Play Out).")]
        public UnityEvent onAnimationStart = new UnityEvent();
        [Tooltip("Invoked when all animations finish.")]
        public UnityEvent onAnimationFinish = new UnityEvent();
        ...
        /// </summary>
        public bool IsAnimating => activeAnimations.Count > 0;
        ...
        {
            // Invoke on finish event
            onAnimationFinish?.Invoke();

            // If it was a Play In, we're done (UI is in its final "in" state)
            if (lastRequestedPlayIn)
                return;

            // If it was a Play Out, reset to initial state (only in editor mode, not in runtime)
            if (Application.isPlaying)
            {
                ...
                {
                    Destroy(gameObject);
                    return; // Don't reset if destroying
                }
                ...
                // In runtime, don't reset - let the UI stay in its "out" state
                return;
            }

            // Reset all elements to their initial state after Play Out (editor mode only)
            ResetToInitialState();
        }
